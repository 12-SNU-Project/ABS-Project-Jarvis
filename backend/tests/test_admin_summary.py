...
def test_admin_summary_with_mock(monkeypatch):
    ...
    monkeypatch.setattr("app.services.admin.get_recent_feature_runs", lambda limit: [])
...
def test_admin_summary_with_logs(monkeypatch):
    ...
    monkeypatch.setattr("app.services.admin.get_recent_feature_runs", lambda limit: fake_logs)
    result = get_admin_summary()
